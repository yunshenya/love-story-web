...
impl AppConfig {
    fn load() -> anyhow::Result<Self> {
        Config::builder()
            ...
            .try_deserialize().with_context(|| anyhow!("Could not deserialize config"))
    }

    pub fn server_config(&self) -> &ServerConfig {
        self.server_config.as_ref().unwrap_or(&DEFAULT_SERVER_CONFIG)
    }

    pub fn database_config(&self) -> &DatabaseConfig {
        self.database_config.as_ref().unwrap_or(&DEFAULT_DATABASE_CONFIG)
    }
}
